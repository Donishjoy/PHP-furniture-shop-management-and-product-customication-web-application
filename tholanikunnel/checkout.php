...
return false;
    ...
    if($rows)
        ...
        $result=mysqli_query($conn,$sql);
              if($result)
        {
           $sq="INSERT INTO order_details (`phoneno`) values ($phonenumber)";
           $rs=mysqli_query($conn,$sq);
           if($rs){
            $last = "SELECT orid from order_details ORDER BY orid DESC LIMIT 1 ";
          ...
          if($ress)
          {
           $f=" UPDATE `order_details` SET `grand`=(SELECT SUM(g.amount) FROM `custorder` AS g WHERE g.orid=order_details.orid) WHERE EXISTS (SELECT * FROM custorder AS g WHERE g.orid=order_details.orid)";
